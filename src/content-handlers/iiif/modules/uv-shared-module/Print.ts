export class Print {
  ...
  public printImage(
    ...
    image.onload = function () {
      ...
      var imageWidth =
        $imageElement.width + that.printSourceLeftMarginsInPixels;
      var imageHeight =
        $imageElement.height +
        that.printSourceTextHeightInPixels +
        that.printSourceTopMarginsInPixels +
        that.printSourceBottomMarginsInPixels;

      var whRatio = imageWidth / imageHeight;
      var widthPercentageLandscape =
        that.calculateWidthPercentageForLandscape(whRatio);
      var widthPercentagePortrait =
        that.calculateWidthPercentageForPortrait(whRatio);
  ...
  private getPrintStyles(
    widthPercentageLandscape: number,
    widthPercentagePortrait: number
  ) {
    var fullWidthLandscape = 100;
    var fullWidthPortrait = 100;
    var sourceTextLandscapeStyle =
      this.printSourceTextIdWithHash +
      " { width: " +
      fullWidthLandscape +
      "%; height: " +
      this.printSourceTextHeightInPixels +
      "px; vertical-align:top; margin-left: " +
      this.printSourceLeftMarginsInPixels +
      "px; max-width: 90% } ";
    var sourceTextPortraitStyle =
      this.printSourceTextIdWithHash +
      " { width: " +
      fullWidthPortrait +
      "%; height: " +
      ...
      this.printSourceLeftMarginsInPixels +
      "px; max-width: 90% } ";
    var imageLandscapeStyle =
      this.printImageIdWithHash +
      " { width: " +
      widthPercentageLandscape +
      "%; vertical-align: top; margin-top: 20px; margin-left: " +
      this.printSourceLeftMarginsInPixels +
      "px; }";
    var imagePortraitStyle =
      this.printImageIdWithHash +
      " { width: " +
      widthPercentagePortrait +
      "%; vertical-align: top; margin-top: 20px; margin-left: " +
      this.printSourceLeftMarginsInPixels +
      "px; }";
    var hideUVContainer = this.UVContainerIdWithHash + " { display:none; } ";
    var landscapeStyle =
      sourceTextLandscapeStyle + imageLandscapeStyle + hideUVContainer;
    var portraitStyle =
      sourceTextPortraitStyle + imagePortraitStyle + hideUVContainer;

    var styleArray = new Array();
    var pageStyle = " @page { margin: 0mm; } ";
    if (
      Math.floor(widthPercentageLandscape * 297) >=
      Math.floor(widthPercentagePortrait) * 210
    )
      styleArray.push(
  ...
  private printIframe(
    ...
    var printStyles = this.getPrintStyles(
      widthPercentageLandscape,
      widthPercentagePortrait
    );
    var htmlContent = this.getHtmlContent()[0].outerHTML;
    var iframeContent = this.getIframeContent(
      document.title,
      printStyles,
      htmlContent
    );
